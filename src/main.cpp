...
#if defined(NDEBUG)
# error "Recyclecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x93a6e237b6d9246f10be749410b8e5f3c8db62fcb168f649a990b3d6c4f97cb6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Recyclecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Recyclecoin Signed Message:\n";
...
{
    // Recyclecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Recyclecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Recyclecoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Recyclecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Recyclecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Recyclecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4186021fd9fa501487ac430bb67f290dfe8138e990fa0d7cb8df77b26b38670f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "22/12/2022 - Recycle Coin hits over a million views";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042ddb1ded947d1bdf97a10e89f94189158feff6dbef775c90f52dd4b85a4b0a8e086df664838a6d7ce20316bc90907573e880eff327bca6746d7aeec555a119b4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1671888808;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086383975;

        if (fTestNet)
        {
            block.nTime    = 1671888799;
            block.nNonce   = 385724257;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xe30acddfd3aa22694128d6dd4f84610c93a7a185ac022cf5366e6db2434b0cd6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xba, 0xd7 }; // Recyclecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RecyclecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RecyclecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RecyclecoinMiner(CWallet *pwallet)
{
    printf("RecyclecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("recyclecoin-miner");
    ...
    try { loop {
        ...
        printf("Running RecyclecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RecyclecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RecyclecoinMiner, pwallet));
}
